...
- Montage: `data/processed/probe_person_noscreen_audit_cwu/montage_fp_cases.png` (red text is the probability of record)
- Grad-CAM: not produced (--no-gradcam).

## 4. Reading
- At the int8 deployment threshold of 0.55, FP is **14.5%**, which is moderate. There is some context shift, so more diverse real-scene people negatives are worth adding. Also check whether the false-positive cases are mostly borderline images such as partially visible screens or reflections.
