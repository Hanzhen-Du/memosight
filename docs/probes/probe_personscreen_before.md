...
## 1. 防泄漏核对
- 探针总数 51；剔除泄漏 0 张（Pexels-ID 撞 0 / 感知近重复 0）；**干净探针 51 张**用于 FP 测。
...
## 3. FP 案例（人眼复核）
- 拼图：`data/processed/probe_personscreen_before/montage_fp_cases.png`（红字=p(记)）
- Grad-CAM：未产出（--no-gradcam）。

## 4. 读数
- ⚠ 干净探针仅 51 张，FP 率统计噪声大，先补到 ~200 再下结论。
- int8 部署阈值(0.55)下 FP **60.8%**：**偏高**。结合审计 gap≈0 ⇒ 更像**协变量/语境偏移**（影棚人像负类不覆盖杂乱实景人），解法是**增负类人像的场景多样性**而非单纯增量。看 Grad-CAM 是否锁人确认。
