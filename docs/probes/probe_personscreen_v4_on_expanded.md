...
| 0.45 | 141/181 | **77.9%** |
| 0.5 | 133/181 | **73.5%** |
| 0.55 | 125/181 | **69.1%**  ← 部署 |

## 3. FP 案例（人眼复核）
- 拼图：`data/processed/probe_personscreen_audit_v4/montage_fp_cases.png`（红字=p(记)）
- Grad-CAM：未产出（--no-gradcam）。

## 4. 读数
- int8 部署阈值(0.55)下 FP **69.1%**：**偏高**。结合审计 gap≈0 ⇒ 更像**协变量/语境偏移**（影棚人像负类不覆盖杂乱实景人），解法是**增负类人像的场景多样性**而非单纯增量。看 Grad-CAM 是否锁人确认。
